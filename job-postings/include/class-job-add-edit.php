    			...
    			switch ( $type ) {
    				...
    				case 'empty_hiring_logo':
						# INPUT
							...
							$global_hiring_organization = esc_html($global_hiring_organization);
								...
								echo '<div class="jfw_repeater job_remote_repeater">';
									...
									foreach ($job_remote_data as $key => $value) {
										echo '<div class="jfw_repeater_row clearfix" data-repeater-item>';
											...
											$val = isset($job_remote_data[$key]['type']) ? esc_html($job_remote_data[$key]['type']) : '';
											...
											$val2 = isset($job_remote_data[$key]['name']) ? esc_html($job_remote_data[$key]['name']) : '';
					...
					case 'checkboxes':
						...
						if( !empty($options) ){
							...
							foreach ($options as $option_key => $option_name) {
								$checked = '';
								$option_key = esc_html($option_key);
							...
							if( is_array($value) ){
								$other_input_value = isset($value['other_input']) ? esc_html($value['other_input']):"";
							}else{
			...
			if( !empty($rows) ){
				$out .= '<datalist id="datalist-'.esc_attr($key).'">';
					foreach ($rows as $key => $row) {
						$out .= '<option value="'.esc_html($row->meta_value).'" />';
					}
				$out .= '</datalist>';
				return $out;
			}
